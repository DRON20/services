...
def getConfig ():
	...
	if (os.path.exists("python/auto_server/auto_server.ini")):
		parser.read("python/auto_server/auto_server.ini")
	else:
		print("auto_server.ini don`t exists")
		sys.exit()
	config = {}
...
logging.basicConfig(format='%(asctime)s: %(levelname)s: %(message)s', filename='python/auto_server/auto_server.log', level=logging.DEBUG)
...
def getPredictions(imagefile):
    ...
    out = net.forward()
    out_prob = out['prob'][0]
    top = out_prob.argsort()[::-1]
    
    for k in top:
	if k <= len(labels):
	    print k, " ", labels[k-1], " ", out_prob[k]
